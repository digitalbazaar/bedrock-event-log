...
var logger = bedrock.loggers.get('app').child('bedrock-event-log');
...
EventLog.prototype._init = function(options, callback) {
  ...
  logger.debug('creating log: ' + self.name);
  async.auto({
    ...
    if(!err) {
      logger.debug('created log: ' + self.name);
    }
    callback(err);
  });
};
...
EventLog.prototype.add = function(event, callback) {
  ...
  if(!validation.valid) {
    ...
    logger.error('validation error', {error: validationError});
  ...
  async.auto({
    ...
    if(err) {
      logger.error('add error', {error: err});
    }
